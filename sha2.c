...
VOID_RETURN sha512_compile(sha512_ctx *ctx)
{
    uint64_t    v[8] = {0}, *p = ctx->wbuf;
    char        j;
...
INT_RETURN  lfs_sha2(const unsigned char data[], unsigned long len, unsigned char hval[])
{
    return sha2(hval, 32, data, len);
}
#endif
