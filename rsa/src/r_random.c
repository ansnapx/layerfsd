...
int __stdcall R_RandomInit(random)
...
int __stdcall R_RandomUpdate(random, block, len)
...
int __stdcall R_GetRandomBytesNeeded(bytesNeeded, random)
...
int __stdcall R_GenerateBytes(block, len, random)
...
void __stdcall R_RandomFinal(random)
R_RANDOM_STRUCT *random;        /* random structure */
{
...
*/
#ifdef RSA_WINDOWS_DRIVER
void __stdcall R_RandomCreate(random)
R_RANDOM_STRUCT *random;                                /* random structure */
{
	ULONG cnow;
	LARGE_INTEGER t;
	...
	while(random->bytesNeeded) {
		cnow = KeQueryTimeIncrement();                 /* use for seed data */
		KeQuerySystemTime(&t);
		RtlTimeToTimeFields(&t, &gmt);
		temp = subrand(cnow);              /* use special routine to produce seed */

		R_RandomUpdate(random, (POINTER)&temp, sizeof(ULONG));
		R_RandomUpdate(random, (POINTER)&gmt, sizeof(TIME_FIELDS));
		R_RandomUpdate(random, (POINTER)&cnow, sizeof(ULONG));
	}

    /* Clean Up time related data */
	R_memset((POINTER)&gmt, 0, sizeof(TIME_FIELDS));
	R_memset((POINTER)&t, 0, sizeof(LARGE_INTEGER));
	cnow = 0;
    temp = 0;
}
#else
...
*/
#ifdef RSA_WINDOWS_DRIVER
void __stdcall R_RandomMix(random)
R_RANDOM_STRUCT *random;
{
    ...
    for(i = 0; i < MIX_CNT; i++) {
		random->state[i] ^= (unsigned char)KeQueryTimeIncrement();
		KeQuerySystemTime(&gmt);
        random->state[15-i] ^= (unsigned char)gmt.QuadPart;
	}
